...
3. NATURAL JOIN inner joins identify matching column names.
	...
	IMP: Natural joins only work if the column you’re joining by has the same name in both tables.	*/
	
	SELECT boys.boy, toys.toy
	FROM boys NATURAL JOIN toys;		--Does not require any ON/WHERE condition
